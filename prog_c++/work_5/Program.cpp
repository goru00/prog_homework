...
template<class T>
Node<T>* tree<T>::deletenode(T& data)
{
...
template<class T>
void tree<T>::update(T& item1, T& item2)
{
    Node<T>* current = root;
    while (current != NULL)
    {
...
public:
    ...
    bool operator <(storage temp) { return (price < temp.price) || ((price == temp.price) && (org < temp.org)); }
    bool operator >(storage temp) { return (price > temp.price) || ((price == temp.price) && (org > temp.org)); }
    bool operator ==(storage temp) { return (num_contract == temp.num_contract); }
...
bool operator<(const storage& arg1, const storage& arg2) {
    return (arg1.org < arg2.org) || ((arg1.org == arg2.org) && (arg1.price < arg2.price));
}
bool operator>(const storage& arg1, const storage& arg2) {
    return (arg2.org < arg1.org) || ((arg2.org == arg1.org) && (arg2.price < arg1.price));
}
bool operator==(const storage& arg1, const storage& arg2) {
    return (arg1.num_contract == arg2.num_contract);
}
...
int main() {
    ...
    const int n = 6;
    storage tab[] = {
        {"2018-01", "ООО «Топаз»", "г.Котельники, 2-ой проезд, д.23", 350000},
    ...
    trees->show();
    storage find_element = { "2020-01", "", "", 1200000 };
    if ((nd = trees->find(find_element)) == NULL) cout << "Данная строка не найдена" << endl;
    else {
        cout << nd->getData() << endl;
    }
    find_element = { "2020-01", "ОАО «Спартак»", "г.Москва, Кривоколенный пер. -14 стр.3", 1200000 };
    if ((nd = trees->find(find_element)) == NULL) cout << "Данная строка не найдена" << endl;
    else {
    ...
    find_element = { "2019-05", "ОАО «Вымпел»", "г.Москва, Таганская -23", 3500000 };
    storage change("2019-05", "ООО «Рога и копыта»", "г.Москва, Таганская -23", 1500000);
    if ((nd = trees->find(find_element)) == NULL) {
        cout << "Данная строка не найдена. Она будет добавлена" << endl;
    ...
    if (trees->empty()) cout << "Дерево пустое" << endl;
